...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function openModal(pet) {
    ...
    // !!! Прописаниие путей к картинке в модальном окне!!!
     // Определить, на какой странице находится пользователь
     // Если на главной странице, добавить 'shelter/' перед путем к картинке.  const isIndexPage = window.location.pathname.endsWith('index.html');  const imagePathPrefix = isIndexPage ? 'shelter/' : './'; Изменила, т к разделила страницы js для каждого html
    const imagePathPrefix = 'shelter/';
  ...
  // Бесконечная карусель с рандомным индексом начала карусели.
 
let currentIndex = Math.floor(Math.random() * petObj.length); // Случайный начальный индекс

// Функция для рендеринга карточек

function renderPets() {
  const container = document.querySelector('.container-article-pet-description'); // Находим контейнер с картами
  if (!container) {
    console.log('Контейнер не найден!');
    return;
}

  const petElements = container.querySelectorAll('.pet-description'); // Находим все карты на странице 
  

  // Определение пути к изображению в зависимости от текущей страницы.  // Определить, на какой странице находится пользователь -  const isIndexPage = window.location.pathname.endsWith('index.html');
     // Если на главной странице, добавить 'shelter/' перед путем к картинке  const imagePathPrefix = isIndexPage ? 'shelter/' : './'; Убрала, т к разделила js файлы по разным html.
  const imagePathPrefix = 'shelter/';
...
function nextPets() {
    currentIndex = (currentIndex + 3) % petObj.length; // Увеличиваем индекс на 3
    renderPets();
}

// Функция для обработки нажатия на стрелку назад
function prevPets() {
    currentIndex = (currentIndex - 3 + petObj.length) % petObj.length; // Уменьшаем индекс на 3
    renderPets();
}
...
document.querySelector('.arrow.next').addEventListener('click', nextPets);
document.querySelector('.arrow.prev').addEventListener('click', prevPets);
document.querySelector('.arrow.nextSec').addEventListener('click', nextPets);
document.querySelector('.arrow.prevSec').addEventListener('click', prevPets);
